...
// nowplaying-refresh.js (override-safe)
// - Does NOT create timers. It only defines window.fetchNowPlaying,
//   which your index already calls on load/interval/focus.
// - Cleans metadata BEFORE parsing (commas preserved in titles).
// - Works with either #nowPlaying OR #now-playing.
// - Does NOT rely on any globals from index; fully self-contained.
(function () {
  function $(id){
    return document.getElementById(id);
  }
  function getRoot(){
    return $('nowPlaying') || $('now-playing') || null;
  }
  ...
  function normalize(data){
    ...
    // e) Give up guessing: keep full string as title (preserves commas)
  ...
  // Expose a SAFE global that your page already calls on interval/focus
  window.fetchNowPlaying = async function(){
    try{
      ...
      // If we can't reliably split, show idle (don't crash, don't guess)
      if (!artist || !title){ showIdle(); return; }

      // Paint
      paint(artist, title);
    } catch (e){
      console.error('fetchNowPlaying override failed:', e);
      showIdle();
    }
  };
})();
